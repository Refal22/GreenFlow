...
if st.session_state.page == "Dashboard":
    ...
    # Filter data based on the selected road
    ...
    # Compute the percentage of average wait time
    if 'Average Wait Time (s)' in df.columns:
        ...
        st.error("Column 'Average Wait Time (s)' not found in dataset.")
        percentage_wait_time = 0

    # Create three columns layout
    col1, col2, col3 = st.columns([1, 2, 1])

    # Column 1: Busiest Lane Indicator
    with col1:
        ...
        if not df_filtered.empty and 'Vehicle Count' in df_filtered.columns:
            ...
            # Average wait time percentage indicator
            ...
            st.error("No data available for selected road.")
    ...
    with col2:
        ...
        if not df_filtered.empty and 'Vehicle Count' in df_filtered.columns:
            ...
            fig_pie_vehicles.update_traces(marker=dict(colors=['#e0f2e9', '#b2e0d6', '#80c5b5', '#4fb99a', '#26a68a']))
        ...
        if 'Average Wait Time (s)' in df_filtered.columns:
            fig_pie_wait_time = px.pie(df_filtered, values='Average Wait Time (s)', names='ROI')
    ...
    # Column 3: Progress Bars for Congestion Level
    with col3:
        st.markdown(f"<h3 style='text-align: center;'>Congestion Level</h3>", unsafe_allow_html=True)
        ...
        fig_bar = px.bar(filtered_summary_df, x='Street', y=['Total_Vehicles', 'Average_Wait_Time'], barmode='group', labels={'value': 'Count', 'Street': 'Street'})
